...
export default function GenAlphaSneakersPage() {
    ...
    <div className={`${space.className} ${jakarta.className} gradient-bg`}>
      ...
      <main className="min-h-screen px-6 py-16">
        <div className="mx-auto max-w-6xl">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-sm text-[#5b7c99] md:text-base">
                Step up your game with the freshest kicks of the season.
              </p>
            </div>
            <Link
              href="/gen-alpha"
          ...
          <div className="mt-6 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[#00cfe826] bg-white/70 px-4 py-3 text-sm text-[#0a2540] shadow-[0_8px_30px_rgba(0,150,200,0.08)] backdrop-blur-[12px]">
            <div className="flex items-center gap-2">
              <span className="text-xs font-semibold uppercase tracking-[0.2em] text-[#5b7c99]">
                Filter
              </span>
              <select
                value={selectedTag}
                onChange={(event) => setSelectedTag(event.target.value)}
                className="rounded-lg border border-[#00cfe826] bg-white/80 px-3 py-1.5 text-sm font-medium text-[#0a2540] outline-none"
              >
                {tagOptions.map((tag) => (
